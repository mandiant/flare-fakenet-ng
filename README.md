...
To stop FakeNet-NG and close out the generated PCAP and HTML UI file simply press `CTRL-C`:
...
With each session of FakeNet-NG, an HTML report is generated containing the Network-Based Indicators captured throughout the session. To terminate the FakeNet session, the user can press `CTRL-C` as mentioned in the `Simple Run` section. Upon termination, this HTML file will be generated in the main working directory of FakeNet. The user can then open the HTML file in any browser to review the NBI summary data from the session.

The HTML report serves as an interactive Graphical User Interface (GUI) that presents the NBI summary in a user-friendly manner. It includes various features to select, filter, and copy NBIs, making malware report generation easier. The UI organizes all NBIs based on their process information and then further categorizes them by the Application Layer Protocol they use.
 ...
 * Additional Information: This cell provides extra information about each NBI request, such as the Transport Layer Protocol used, destination IP, port, and SSL encryption status.
